 ...
 * @version 2.1.1.0
 * @description 🛡️ DCV NEXSUS Panel: Discord Görev Tamamlayıcı, HypeSquad Değiştirici, 11 Siber Tema, 4D Blok Butonlar, 24h Zamanlayıcı, Sonsuz Kronometre, Canlı Saat, Snake Oyunu, Panel Ayarları ve Discord Araçları.
 * @source https://github.com/cirpanemir039-ops/DCV-NEXSUS-Panel
 * @updateUrl https://raw.githubusercontent.com/cirpanemir039-ops/DCV-NEXSUS-Panel/main/DCVNexsusPanel.plugin.js
 */

module.exports = class DCVNexsusPanel {
    start() {
        this.injectPlugin();
    }
...
╚══════════════════╝">VIP Rozet Kutusu</button>
    ...
    if (document.readyState === 'loading') {
        ...
        injectUI();
            ...
            console.error('[DCV-NEXSUS] Plugin start error:', e);
        }
    }

    cleanupPlugin() {
        try {
            const root = document.getElementById('discord-quest-ui-root');
            if (root) root.remove();
            const trigger = document.getElementById('dqu-floating-trigger');
            if (trigger) trigger.remove();
            const styles = document.querySelectorAll('style[id^="discord-quest-"]');
